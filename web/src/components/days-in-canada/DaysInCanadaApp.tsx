...
import { dedupeTrips, mergeParsedIntoTrips } from "@/lib/days-in-canada/trips";
...
function StepIndicator({ current, steps }: { current: StepId; steps: { id: StepId; label: string }[] }) {
  ...
  return (
    <nav aria-label="Progress" className="days-gone-stepper days-gone-no-print">
          ...
          return (
            <li key={step.id} className="flex min-w-0 flex-1 items-center last:flex-none">
              <div className="flex min-w-0 flex-col items-center gap-1.5 sm:gap-2">
                <div
                  className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-[11px] font-semibold transition sm:h-8 sm:w-8 sm:text-xs ${
                    active
                      ? "bg-claude-accent text-[var(--dg-band-text)] ring-2 ring-claude-accent/15 sm:ring-4"
                      : done
                ...
                <span
                  className={`hidden max-w-[4.5rem] truncate text-center text-[10px] leading-tight sm:max-w-none sm:text-xs md:block ${
                    active ? "font-medium text-claude-text" : "text-claude-muted"
                  }`}
                >
                  {step.label}
                </span>
              ...
              {i < steps.length - 1 ? (
                <div
                  className={`mx-1 mb-5 h-px min-w-[0.35rem] flex-1 sm:mx-2 sm:mb-6 ${done ? "bg-claude-accent/50" : "bg-claude-border"}`}
                />
              ) : null}
            </li>
          );
...
function StepFooter({
  ...
  return (
    <div className="days-gone-no-print mt-8 border-t border-claude-border pt-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
        {onBack ? (
          <button type="button" onClick={onBack} className={`${btnSecondary} w-full sm:w-auto`}>
            {backLabel ?? "Back"}
          </button>
        ) : null}
        {hint ? <p className="text-xs text-claude-muted sm:order-3 sm:ml-auto">{hint}</p> : null}
        <button
          type="button"
          onClick={onNext}
          disabled={nextDisabled}
          className={`${btnPrimary} w-full sm:ml-auto sm:w-auto`}
        >
          {nextLabel}
        </button>
      </div>
...
function SegmentedControl<T extends string>({
  ...
  return (
    <div className="flex w-full flex-col gap-1 rounded-lg border border-claude-border bg-[var(--dg-tint)] p-1 sm:inline-flex sm:w-auto sm:flex-row">
      {options.map((opt) => (
        <button
          key={opt.value}
          type="button"
          onClick={() => onChange(opt.value)}
          className={`rounded-md px-3 py-2 text-left text-sm font-medium transition sm:px-4 sm:text-center ${
            value === opt.value
      ...
      <div className="mt-3 space-y-4 border-t border-claude-border/80 pt-3">
        <div className="space-y-2">
          ...
          <textarea
            ...
            className={input}
          />
          <button type="button" onClick={onFindMissingLeg} className={`${btnPrimary} w-full sm:w-auto`}>
...
export function DaysInCanadaApp() {
  ...
  const [, setHasParsed] = useState(false);
  ...
  const [, setFetchedTrips] = useState<Trip[]>([]);
  ...
  return (
    <div>
      ...
      <div className="days-gone-band border-b border-claude-border">
        <DaysGoneBackdrop />
        <div className="relative z-10 mx-auto w-full max-w-[640px] px-4 pb-6 pt-7 sm:px-6 sm:pb-8 sm:pt-10 md:pt-12">
          <header className="mb-6 flex flex-col gap-3 sm:mb-8 sm:flex-row sm:items-start sm:justify-between sm:gap-4">
            <div className="min-w-0">
              <p className="font-mono text-[10px] uppercase tracking-[0.1em] text-claude-accent sm:text-[11px] sm:tracking-[0.12em]">
                Free · Private · Browser-only
              </p>
              <h1 className="mt-1.5 font-sans text-[1.35rem] font-bold uppercase tracking-[0.1em] text-claude-text sm:mt-2 sm:text-[1.65rem] sm:tracking-[0.14em] md:text-[1.85rem]">
                Days Gone
              </h1>
              <p className="mt-1 text-xs leading-relaxed text-claude-muted sm:mt-1.5">
      ...
      <div className="mx-auto w-full max-w-[640px] px-4 pb-10 pt-8 sm:px-6 sm:pt-10">
        {step === "welcome" ? (
                ...
                <div className={`${panel} p-5 sm:p-6`}>
                  ...
                  <div className="mt-4">
                    <button
                      ...
                      className={`w-full sm:w-auto ${isParsing ? btnDisabled : btnPrimary}`}
                    >
        ...
        {step === "results" ? (
          <div className="space-y-6">
            ...
            {exportOptions ? (
              <div className="days-gone-no-print grid grid-cols-1 gap-2 sm:flex sm:flex-wrap">
                <button type="button" onClick={handleExportCsv} className={`${btnSecondary} w-full sm:w-auto`}>
                  Export CSV
                </button>
                <button
                  type="button"
                ...
                <button type="button" onClick={() => window.print()} className={`${btnSecondary} w-full sm:w-auto`}>
                  Print
                </button>
              </div>
            ) : null}
        ...
        <footer className="days-gone-no-print mt-14 border-t border-claude-border pt-6 text-center text-xs text-claude-muted">
          <Link to="/" className="hover:text-claude-accent">
            arshiya.dev
          </Link>
          {" · "}
